...
async function init() {
    ...
    document.getElementById('edit-category').innerHTML = buildOptions();
    await fetchSales();
}
...
async function fetchSales() {
    ...
    try {
        ...
        sales = data.map(i => ({ id:i.id, t_id:i.transaction_id, category:i.category, price:i.price, quantity:i.quantity, image:i.image_data, date:i.sale_date }));
        const groups = {};
        ...
        transactions = sorted.map((tid,i) => {
            ...
            return { id:tid, display_id:i+1, items, total_amount:items.reduce((s,x)=>s+(x.price*x.quantity),0), items_count:items.reduce((s,x)=>s+x.quantity,0), date:items[0].date };
        });
        transactions.sort((a,b)=>new Date(b.date)-new Date(a.date));
        if (statusEl) { statusEl.textContent='● Connected'; statusEl.style.color='#10b981'; }
        renderSales(); updateStats();
    } catch(err) {
...
function renderSales(filter='') {
    ...
    if (fd) list = list.filter(t => toLocalDate(t.date) === fd);

    if (!list.length) {
        salesBody.innerHTML = `<tr><td colspan="5"><div class="empty-state"><div class="empty-icon">🛍️</div><div class="empty-title">${fd?`No sales on ${formatDate(fd)}`:'No sales yet'}</div><div class="empty-sub">${fd?'Try a different date or clear the filter.':'Click "Add New Sale" to get started.'}</div></div></td></tr>`;
        return;
    }
    salesBody.innerHTML = '';
    list.forEach(t => {
        ...
        tr.innerHTML = `<td style="font-weight:700">Sale #${t.display_id}</td><td style="font-weight:700;color:var(--accent-secondary)">₹${t.total_amount.toLocaleString('en-IN')}</td><td><span class="category-tag">${t.items_count} Items</span></td><td style="color:var(--text-secondary);font-size:.875rem">${formatDate(t.date)}</td><td style="text-align:center"><button onclick="deleteTransaction('${t.id}')" class="action-btn action-btn-delete"><i data-lucide="trash-2" style="width:16px"></i> Delete</button></td>`;
...
function updateStats() {
    const fd = filterDateInput.value;
    const filtered = fd ? sales.filter(s => toLocalDate(s.date) === fd) : sales;
    ...
    const label = fd ? `on ${formatDate(fd)}` : null;
    revenueLabelEl.textContent = label ? `Revenue ${label}` : 'Total Revenue';
    revenueLabelEl.className = `stat-label${fd?' filtered':''}`;
    salesLabelEl.textContent = label ? `Sales ${label}` : 'Total Sales';
    salesLabelEl.className = `stat-label${fd?' filtered':''}`;
    categoryLabelEl.textContent = label ? `Top Category ${label}` : 'Top Category';
    categoryLabelEl.className = `stat-label${fd?' filtered':''}`;
...
function openSaleDetails(tid) {
    ...
    document.getElementById('details-sale-date').textContent = formatDate(t.date);
    ...
    document.getElementById('details-items-body').innerHTML = t.items.map(item=>`
        <tr style="border-bottom:1px solid rgba(255,255,255,.05)">
            ...
            <td>₹${item.price.toLocaleString('en-IN')}</td><td>${item.quantity}</td>
...
window.deleteTransaction = async tid => {
    ...
    try {
        ...
        if (r.ok) { await fetchSales(); showToast('Sale deleted.'); }
...
addSaleBtn.onclick = () => {
    ...
    updateGrandTotal(); modalOverlay.style.display='flex'; lucide.createIcons();
};
...
saleForm.onsubmit = async e => {
    ...
    document.querySelectorAll('.line-item').forEach(row => {
        ...
        if(price>0&&qty>=1) items.push({category:row.querySelector('.item-category').value,price,quantity:qty,date,image:currentImageBase64});
    });
    ...
    try {
        ...
        if(r.ok){await fetchSales();closeModalAction();showToast(`${items.length} item(s) recorded! 🎉`);}
...
filterDateInput.onchange = () => {
    clearDateBtn.style.display = filterDateInput.value ? 'flex' : 'none';
    updateStats(); renderSales(searchInput.value);
    if(document.getElementById('pane-analytics').style.display!=='none') setTimeout(()=>{renderCharts();analyzePatterns();},50);
};
clearDateBtn.onclick = () => {
    filterDateInput.value=''; clearDateBtn.style.display='none';
    updateStats(); renderSales(searchInput.value);
    if(document.getElementById('pane-analytics').style.display!=='none') setTimeout(()=>{renderCharts();analyzePatterns();},50);
};
...
window.onclick = e => {
    ...
    if(e.target===document.getElementById('edit-sale-overlay')) document.getElementById('edit-sale-overlay').style.display='none';
...
function getChartSales() {
    const fd=filterDateInput.value;
    return fd ? sales.filter(s=>toLocalDate(s.date)===fd) : sales;
}

function renderCharts() {
    const cs=getChartSales(),today=new Date(),rL=[],rD=[];
    for(let i=29;i>=0;i--){
        const d=new Date(today); d.setDate(d.getDate()-i);
        const ds=toLocalDate(d.toISOString());
        rL.push(d.toLocaleDateString('en-IN',{month:'short',day:'numeric'}));
        rD.push(cs.filter(s=>toLocalDate(s.date)===ds).reduce((a,s)=>a+s.price*s.quantity,0));
    }
    ...
    const cm={};cs.forEach(s=>cm[s.category]=(cm[s.category]||0)+s.price*s.quantity);
    const cl=Object.keys(cm);
    if(chartCategory) chartCategory.destroy();
    chartCategory=new Chart(document.getElementById('chart-category'),{type:'doughnut',data:{labels:cl,datasets:[{data:cl.map(k=>cm[k]),backgroundColor:CHART_COLORS.slice(0,cl.length),borderWidth:2,borderColor:'#0f172a'}]},options:{responsive:true,cutout:'65%',plugins:{legend:{position:'right',labels:{color:TC,padding:12,font:{size:11}}}}}});
    const dl=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],dc=[0,0,0,0,0,0,0];
    cs.forEach(s=>dc[new Date(s.date).getDay()]++);
...
function analyzePatterns() {
    const grid=document.getElementById('insight-grid'),cs=getChartSales();
    if(!cs.length){grid.innerHTML='<div class="insight-placeholder">Add some sales to see patterns emerge...</div>';return;}
    const totalRev=cs.reduce((s,x)=>s+x.price*x.quantity,0);
    const catRev={};cs.forEach(s=>catRev[s.category]=(catRev[s.category]||0)+s.price*s.quantity);
    ...
    const twR=cs.filter(s=>new Date(s.date)>=w1).reduce((a,s)=>a+s.price*s.quantity,0);
    const lwR=cs.filter(s=>new Date(s.date)>=w2&&new Date(s.date)<w1).reduce((a,s)=>a+s.price*s.quantity,0);
    const trend=lwR?(((twR-lwR)/lwR)*100).toFixed(1):null;
    const dc2=[0,0,0,0,0,0,0];cs.forEach(s=>dc2[new Date(s.date).getDay()]++);
    const peakDay=['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][dc2.indexOf(Math.max(...dc2))];
    const catCnt={};cs.forEach(s=>catCnt[s.category]=(catCnt[s.category]||0)+1);
    const mostActive=Object.entries(catCnt).sort((a,b)=>b[1]-a[1])[0];
    const pct=((topCat[1]/totalRev)*100).toFixed(0);
    const insights=[
        {icon:'🏆',label:'Top Revenue Category',value:topCat[0],sub:`₹${topCat[1].toLocaleString('en-IN')} earned`,color:'#8b5cf6'},
        {icon:trend&&parseFloat(trend)>=0?'📈':'📉',label:'Weekly Trend',value:trend?`${parseFloat(trend)>=0?'+':''}${trend}%`:'First week!',sub:`₹${twR.toLocaleString('en-IN')} vs ₹${lwR.toLocaleString('en-IN')}`,color:(!trend||parseFloat(trend)>=0)?'#10b981':'#ef4444'},
        {icon:'⏰',label:'Peak Sales Day',value:peakDay,sub:`${dc2[['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'].indexOf(peakDay)]} sale(s) historically`,color:'#06b6d4'},
        {icon:'💰',label:'Avg. Order Value',value:`₹${(totalRev/cs.length).toFixed(0)}`,sub:`Across ${cs.length} sale(s)`,color:'#f59e0b'},
        {icon:'🔥',label:'Most Active Category',value:mostActive[0],sub:`${mostActive[1]} sale(s)`,color:'#f97316'},
        {icon:'💡',label:'Recommendation',value:'Insight',sub:`${topCat[0]} drives ${pct}% of revenue.${parseInt(pct)>50?' Consider diversifying.':' Growing others could boost income.'}`,color:'#3b82f6',wide:true}
    ];
...
document.getElementById('close-edit-modal').onclick=()=>editSaleOverlay.style.display='none';
document.getElementById('edit-sale-form').onsubmit=async e=>{
    e.preventDefault();
    const id=document.getElementById('edit-sale-id').value;
    const data={category:document.getElementById('edit-category').value,price:parseFloat(document.getElementById('edit-price').value),quantity:parseInt(document.getElementById('edit-quantity').value),date:document.getElementById('edit-sale-date').value};
    try{
        const r=await fetch(`${API_BASE}/sales/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
        if(r.ok){editSaleOverlay.style.display='none';await fetchSales();showToast('Sale updated.');}
...
window.openEditModal=id=>{
    ...
    if(s.date) document.getElementById('edit-sale-date').value=s.date.split('T')[0];
    editSaleOverlay.style.display='flex'; lucide.createIcons();
};
...
document.getElementById('save-img-btn').onclick=async()=>{
    ...
    try{
        ...
        if(r.ok){editImgOverlay.style.display='none';await fetchSales();showToast('Image updated.');}
        else showToast('Failed to update image.','error');
    }catch{showToast('Connection failed.','error');}
};
window.deleteSale=async id=>{
    if(!confirm('Delete this sale?'))return;
    try{const r=await fetch(`${API_BASE}/sales/${id}`,{method:'DELETE'});if(r.ok){await fetchSales();showToast('Sale deleted.');}else showToast('Error.','error');}
    catch{showToast('Connection failed.','error');}
};
